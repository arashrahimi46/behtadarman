	...
	class Puc_v4p4_Plugin_UpdateChecker extends Puc_v4p4_UpdateChecker {
		...
		public function addViewDetailsLink($pluginMeta, $pluginFile, $pluginData = array()) {
			...
			if ( $isRelevant && $this->userCanInstallUpdates() && !isset($pluginData['slug']) ) {
				$linkText = apply_filters($this->getUniqueName('view_details_link'), __('View changelog'));
				if ( !empty($linkText) ) {
					...
					echo $viewDetailsLinkPosition;

					$viewDetailsLink = sprintf('<a href="%s"  aria-label="%s" data-title="%s" target="_blank">%s</a>',
						esc_url('https://digits.unitedover.com/changelog'),
