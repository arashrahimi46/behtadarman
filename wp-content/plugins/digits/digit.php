...
/*
 * Plugin Name: Digits: ورود و ثبت نام از طریق پیامک به موبایل
 * Description: ورود و عضویت کاربران به سایت وردپرس/ووکامرس از طریق ارسال پیامک تایید به موبایل
 * Version: Latest
 * Plugin URI: https://IPPanel.com
 * Author URI: https://IPPanel.com
 * Author: IPPanel Digit
 * Text Domain: digits
 * Requires PHP: 5.5
...
function getCountry()
{
    ...
    $countrycode_default = getCountryCode(get_option("dig_default_ccode"));

    if ($countrycode_default != -1) {
        return getCountryCode($countrycode_default);
    }
...
function createCustomerOnOrderPage($noui = false)
{
    ...
    wp_enqueue_style('digits-cco-style', plugins_url('assests/css/dig_cco.css', __FILE__));
...
?>
    ...
    <?php if (!is_user_logged_in()) { ?>
        ...
        <div id="dig-ucr-container" class="dig_ma-box dig-box <?php echo $dig_main_re;
        if ($page_type == 2) echo ' dig_pgmdl_2'; ?>" style="display:none;">


            <div class="dig-content dig-modal-con <?php if ($page_type == 2) echo 'dig_ul_divd';
            echo ' ' . $theme; ?>">
                <?php if ($page_type == 2) {
                                    ...
                                    <div class="dig_reg_inputs">
                                        ...
                                        if ($passaccep > 0) {
                                            ...
                                            <div id="dig_cs_password"
                                                 class="minput" <?php if ($passaccep == 1) echo 'style="display: none;"'; ?>>
                                        ...
                                        if ($mobileaccp > 0 || $passaccep == 0 || $passaccep == 2) {
                                            ...
                                            <button class="<?php echo $themee . ' ' . $bgtype; ?> button dig-signup-otp registerbutton"
                                                    value="<?php echo $subVal; ?>"
                                                    type="submit"><?php echo $subVal; ?></button>
                ...
                $ms = '';
                if (empty($digpc)) $ms = "<a class='digmsg-pow' href='https://1.envato.market/c/1271986/275988/4415?subId1=poweredby&u=https%3A%2F%2Fcodecanyon.net%2Fitem%2Fdigits-wordpress-mobile-number-signup-and-login%2F19801105'> " . __('Powered by <span>Digits</span>', 'digits') . "</a>";
...
function dig_login_contents($modal, $type = 1, $page = false)
{
    ...
    if (!is_user_logged_in()) {
        ...
        if ($type == 1) {
            return '<a href="?login=true' . dig_url_language() . '" ' . $element . ' ' . $opatt . ' type="' . $dtype . '"><span>' . $diglogintrans . '</span></a>' . $modalBox;
        } else if ($type == 2) {
            return '<a href="?login=true&page=2' . dig_url_language() . '" ' . $element . ' ' . $opatt . ' type="2"><span>' . $digregistertrans . '</span></a>' . $modalBox;
        } else if ($type == 3) {
            return '<a href="?login=true&page=3' . dig_url_language() . '" ' . $element . ' ' . $opatt . ' type="3"><span>' . $digforgottrans . '</span></a>' . $modalBox;
        } else if ($type == 4) {
            return '<a href="?login=true&page=4' . dig_url_language() . '" ' . $element . ' ' . $opatt . ' type="4"><span>' . $digonlylogintrans . '</span></a>' . $modalBox;
        }
...
// Returns a string if the URL has parameters or NULL if not
        ...
        return "<a href='" . $url . dig_url_language() . "' type='10' class=\"digits-login-modal\"><span>" . __($logouttrans, "digits") . "</span></a>";
    }
}
...
function dig_add_gs_css()
{
    ...
    if (is_rtl()) {
        wp_enqueue_style('digits-gs-rtl-style', plugins_url('/assests/css/gs-rtl.css', __FILE__), array('digits-gs-style'), null, 'all');
...
{
    $smsgateways = array(
        'Ippanel' => array('value' => 512, 'inputs' => array(__('Username') => array('text' => true, 'name' => 'username'), __('Password') => array('text' => true, 'name' => 'password'), __('Sender') => array('text' => true, 'name' => 'sender'), __('Send as sample') => array('text' => false, 'name' => 'sample', 'optional' => 1), __('Shop Name') => array('text' => true, 'name' => 'shopname', 'optional' => 1))),
    );
    ...
    <div class="dig_api_test" <?php if ($digit_tapp == 1 || $digit_tapp == 13) echo 'style="display:none;"' ?>>
        ...
        <div id="dig_call_test_api">
            ...
            <div class="dig_test_mob_ho">
                ...
                <input data-dig-mob="1"
                       countryCode="<?php echo esc_attr(get_the_author_meta('digt_countrycode', get_current_user_id())); ?>"
                       dig-save="0" id="username" f-mob="1" reg="2" only-mob="1" type="text"
                       placeholder="<?php _e('Your Mobile Number', 'digits'); ?>" id="dig_test_number"
                       value="<?php echo esc_attr(get_the_author_meta('digits_phone_no', get_current_user_id())); ?>"/>
                            ...
                            <label for="preset<?php echo $key; ?>">
                                <div class="dig_preset_item_list">
                                    <input class="dig_preset" name="dig_preset" id="preset<?php echo $key; ?>"
                                           value="<?php echo $key; ?>"
                                           type="radio" <?php if ($key == $preset) echo 'checked'; ?>>
                                    <div class="dig_preset_sel">
                                        <img class="dig_preset_sel_tick"
                                             src="<?php echo plugins_url('/assests/images/preset-tick.svg', __FILE__); ?>"
                                             draggable="false"/>
                                    </div>
                                    <div class="dig_preset_img_smp">
                                        ...
                                        <a class="dig_preset_big_img"
                                           big-href="<?php echo plugins_url('/assests/images/preset' . $key . '.jpg', __FILE__); ?>">
                                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 64 64"
                                                ...
                                                <path data-name="layer2"
                                                      ...
                                                      stroke-width="2" d="M16 22h32v20H16z"
                                                      stroke-linejoin="round" stroke-linecap="round"></path>
                                                <path data-name="layer1" fill="none" stroke="#ffffff"
                                                      stroke-miterlimit="10"
                                                      stroke-width="2" d="M2 16V2h20m40 14V2H42M2 49v13h20m40-14v14H42"
    ...
    <table class="form-table">
        ...
        <tr class="dig_modal_type_2">
            ...
            <td>
                ...
                ?>
                <div class='image-preview-wrapper'>
                    <img id='bg_image-preview_left_modal'
                         src='<?php if (is_numeric($imgid)) echo $wid; else echo $imgid; ?>'
                         style="max-height:100px;">
                </div>
...
function digits_configure_settings()
{
    ...
    $dig_messagetemplate = get_option("dig_messagetemplate", "%NAME% رمز یکبار مصرف شما %OTP% می باشد.");
        ...
        <tr class="dig_csmargn">
            ...
            <td>
                ...
                <p class="dig_ecr_desc"><?php _e('Leave blank for auto redirect', 'digits'); ?>
                    <br/><b><?php _e('Note:', 'digits'); ?></b>&nbsp;<?php _e('Redirect settings only works on Digits Login/Signup Modal and Page', 'digits'); ?>
                </p>
            </td>
        </tr>
    ...
    <div class="dig_presets_modal" id="dig_export_import_content">
        <div id="dig_presets_modal_box">
...
$DigitsUpdateChecker = Puc_v4_Factory::buildUpdateChecker(
    'https://digits.unitedover.com/updates/changelog/digits.json',
    __FILE__,
    'digits'
);
 ...
 ?>
    ...
    <div class="dig_bdy_container <?php if ($page_type == 2) echo 'dig_ul_divd'; ?>">
        ...
        <div class="dig_ma-box <?php if ($page_type == 2) echo 'dig_pgmdl_2'; ?>">
                            ...
                            <form method="post" class="digits_register">
                                ...
                                <div class="dig_reg_inputs">
                                    ...
                                    if ($passaccep > 0) {
                                        ...
                                        <div class="minput" <?php if ($passaccep == 1) echo 'style="display: none;"'; ?>
                                             id="dig_cs_password">
                                            <input type="password" name="digits_reg_password"
                                                   id="digits_reg_password" <?php if ($passaccep == 2) echo "required"; ?>/>
                                            <label><?php _e("Password", "digits"); ?></label>
            ...
            $style = "";
            if (!empty($backcolor)) {
                $style = 'fill:' . $fontcolor1 . '';
            }
            if (empty($digpc)) {
                ?>
                <div class='dig_powrd' style='opacity: 0;'>
                    <a class='digmsg-pow'
                       href='https://1.envato.market/c/1271986/275988/4415?subId1=poweredby&u=https%3A%2F%2Fcodecanyon.net%2Fitem%2Fdigits-wordpress-mobile-number-signup-and-login%2F19801105'>
                        <svg style="height:18px;<?php echo $style ?>" xmlns="http://www.w3.org/2000/svg"
                             viewBox="0 0 106.52 13.26">
                            <path d="M0,.41H3.5c2,0,3.19,1.09,3.19,2.9S5.52,6.19,3.5,6.19H.83V10H0Zm3.46,5C5,5.4,5.84,4.7,5.84,3.31S5,1.2,3.46,1.2H.83V5.4Z"/>
                            <path d="M7.27,6.36c0-2.27,1.36-3.83,3.35-3.83S14,4.09,14,6.36s-1.36,3.83-3.37,3.83S7.27,8.63,7.27,6.36Zm5.92,0c0-1.86-1-3.09-2.57-3.09S8.07,4.5,8.07,6.36s1,3.09,2.55,3.09S13.19,8.22,13.19,6.36Z"/>
                            <path d="M14.78,2.73h.79l1.4,6h0l2-6h.75l2,6h0l1.39-6H24L22.29,10h-.77L19.4,3.73h0L17.27,10H16.5Z"/>
                            <path d="M24.8,6.36c0-2.24,1.3-3.83,3.27-3.83S31.28,4,31.28,6.19v.37H25.61c0,1.67.95,2.9,2.47,2.9a2.42,2.42,0,0,0,2.37-1.58l.65.38a3.09,3.09,0,0,1-3,1.93C26.1,10.19,24.8,8.68,24.8,6.36Zm5.68-.45a2.48,2.48,0,0,0-2.41-2.66,2.57,2.57,0,0,0-2.45,2.66Z"/>
                            <path d="M32.9,2.73h.75v.7h0a1.63,1.63,0,0,1,1.49-.82,2.19,2.19,0,0,1,.63.1l-.16.8a2.47,2.47,0,0,0-.6-.08c-.89,0-1.34.64-1.34,2V10H32.9Z"/>
                            <path d="M36.33,6.36c0-2.24,1.29-3.83,3.26-3.83S42.81,4,42.81,6.19v.37H37.13c0,1.67,1,2.9,2.48,2.9A2.41,2.41,0,0,0,42,7.88l.66.38a3.1,3.1,0,0,1-3,1.93C37.62,10.19,36.33,8.68,36.33,6.36ZM42,5.91a2.46,2.46,0,0,0-2.41-2.66,2.57,2.57,0,0,0-2.44,2.66Z"/>
                            <path d="M44.13,6.36c0-2.26,1.34-3.83,3.23-3.83a2.71,2.71,0,0,1,2.27,1.12h0V0h.79V10h-.77v-.9h0a2.73,2.73,0,0,1-2.3,1.11C45.47,10.19,44.13,8.63,44.13,6.36Zm5.53.9V5.43a2.22,2.22,0,0,0-2.3-2.16c-1.44,0-2.43,1.26-2.43,3.09s1,3.09,2.49,3.09A2.23,2.23,0,0,0,49.66,7.26Z"/>
                            <path d="M56.82,9.08h0V10H56V0h.79V3.65h0a2.71,2.71,0,0,1,2.27-1.12c1.88,0,3.22,1.57,3.22,3.83S61,10.19,59.12,10.19A2.73,2.73,0,0,1,56.82,9.08Zm4.73-2.72c0-1.83-1-3.09-2.43-3.09a2.22,2.22,0,0,0-2.3,2.16V7.26a2.23,2.23,0,0,0,2.24,2.19C60.55,9.45,61.55,8.2,61.55,6.36Z"/>
                            <path d="M65.57,9.37,62.75,2.73h.85L66,8.45h0l2.36-5.72h.85L64.85,13.06H64Z"/>
                            <path d="M74,.41h3.08c2.46,0,4.14,1.76,4.14,4.77,0,3.27-1.68,4.8-4.19,4.8H74Zm3,8.26c1.76,0,2.84-1,2.84-3.49,0-2.3-1.08-3.46-2.77-3.46H75.44v7Z"/>
                            <path d="M82.52.07H84V1.48H82.52Zm.07,2.66h1.34V10H82.59Z"/>
                            <path d="M85.78,11.36l1.13-.64A1.93,1.93,0,0,0,88.83,12a1.73,1.73,0,0,0,1.77-2v-.8h0a2.49,2.49,0,0,1-2.08.91c-1.9,0-3.24-1.59-3.24-3.83s1.34-3.82,3.25-3.82a2.73,2.73,0,0,1,2.07.89h0v-.7h1.32v7.41a2.92,2.92,0,0,1-3.09,3.12A3.19,3.19,0,0,1,85.78,11.36ZM90.58,7V5.74a1.93,1.93,0,0,0-2-1.95c-1.23,0-2,1-2,2.57s.81,2.58,2.06,2.58A1.92,1.92,0,0,0,90.58,7Z"/>
                            <path d="M93.39.07h1.47V1.48H93.39Zm.07,2.66H94.8V10H93.46Z"/>
                            <path d="M98.26,3.94V8.07c0,.46.18.64.78.64h.66V10h-.78c-1.47,0-2-.51-2-1.89V3.94H95.79V2.73h1.13V1h1.34V2.73H100V3.94Z"/>
                            <path d="M100.5,7.55l1.26-.28a1.8,1.8,0,0,0,2,1.73c.89,0,1.44-.35,1.44-.89s-.54-.95-1.86-1.34c-1.64-.47-2.49-1-2.49-2.14s1.09-2.1,2.6-2.1a2.79,2.79,0,0,1,2.88,2.15L105.08,5a1.57,1.57,0,0,0-1.62-1.28c-.8,0-1.3.35-1.3.89s.45.69,1.62,1c1.77.52,2.74,1.1,2.74,2.42s-1.08,2.14-2.75,2.14A2.94,2.94,0,0,1,100.5,7.55Z"/>
                        </svg>
                    </a></div>
            ...
            if ($type == "dropdown" && $user_role == 1) {
                ...
                <select name="digits_reg_<?php echo $meta_key; ?>" <?php echo $custom_class; ?> <?php if ($values['required'] == 1) echo "required"; ?>>
                    ...
                    foreach ($values['options'] as $option) {
                        ...
                        $option = dig_filter_string($option);
                        if ($e_value == $option) $selected = "selected";
                        echo "<option " . $selected . ">" . __($option, 'digits') . "</option>";
                    }
                    ?>
                </select>
...
function setup_wizard_header()
{
    ...
    <h1 id="digits-logo"><a href="https://digits.unitedover.com" target="_blank">
            <img src="<?php echo plugins_url('assests/images/Digits_logo.png', __FILE__) ?>" alt="Digits"/></a></h1>
...
function digit_activation($form = true)
{
    ...
    <p class="dig_ecr_desc dig_cntr_algn_clr dig_prchcde" <?php if (!empty($code)) echo 'style="display:block;"'; ?>>
        ...
        <b>Digits</b>" <?php _e('and receive updates', 'digits'); ?>.
    </p>
    <?php
    if (!$form)
        return;
