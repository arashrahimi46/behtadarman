...
trait Base {
	...
	public static function set( $key, $value ) {
		if ( ! is_string( $key ) ) {
			trigger_error( __( 'Key passed to `set` method must be key', ETC::PLUGIN_ID ), E_USER_ERROR );
		}
	...
	public static function get( $key ) {
		if ( ! is_string( $key ) ) {
			trigger_error( __( 'Key passed to `get` method must be key', ETC::PLUGIN_ID ), E_USER_ERROR );
		}
