...
class Fancy_Button extends Shortcodes {
	...
	function fancy_button_shortcode( $atts, $content ) {
		...
		if( ! function_exists( 'vc_icon_element_fonts_enqueue' ) && ! function_exists( 'vc_build_link' ) ) return;
	    ...
	    if ( $atts['btn_style'] != '' && $atts['btn_style'] != 'custom' )
	    	$options['button_class'] .= ' ' . $atts['btn_style'];

	    if ( $atts['btn_size'] != '' && $atts['btn_size'] != 'custom' )
	    	$options['button_class'] .= ' ' . $atts['btn_size'];
	    ...
	    // $atts['class'] .= ' align-items-' . $atts['valign'];
        ...
        if ( $options['rs_button_line_height']['mobile'] != '' )
   		...
   		if( trim($options['border_radius']) != '' ) 
   			$options['css'][$options['selectors']['button']][] = 'border-radius:' . $options['icon_border_radius'];
		...
		<div class="et-fancy-button <?php echo $atts['class']; ?>">
			...
			<?php 
				...
				echo sprintf(
					'<div class="button-wrap inline-block"><a href="%s" class="%s" title="%s" target="%s">%s <span>%s</span> %s</a></div>', 
					$options['a_href'],
