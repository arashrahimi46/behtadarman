...
class Slider_Item extends Shortcodes {
	...
	function slider_item_shortcode( $atts, $content ) {
		...
		$options['button_class'] = $options['description_class'] = $options['custom_class'] = $options['img'] = $options['item_style'] = '';
		...
		if ( is_array( $options['img'] ) && count( $options['img'] ) > 0 ) {
			$options['item_style'] .= ' background-image: url('.$options['img'][0].');';
	    ...
	    // backup of title size 
	        if ( empty($options['rs_title_size']['desktop']) && empty($options['rs_title_size']['tablet']) && empty($options['rs_title_size']['mobile']) ) {
        ...
        // backup of title line height 
	        if ( empty($options['rs_title_line_height']['desktop']) && empty($options['rs_title_line_height']['tablet']) && empty($options['rs_title_line_height']['mobile']) ) {
		...
		$options['data_attr']['item'][] = 'class="slider-item fadeIn-slide '.$options['box_id'].' '. esc_attr( $atts['el_class'] ). '"';
		...
		if ( !empty( $atts['description_animation'] ) ) {
		...
		<div <?php echo implode(' ', $options['data_attr']['item']); ?>>
			...
			<div class="container <?php echo esc_attr($options['align_class']); ?>">
				...
				<div class="slide-content <?php echo esc_attr($options['custom_class']); ?>">
					...
					<?php if ( $atts['subtitle_above'] && ! empty( $atts['subtitle'] ) ) 
					...
					if ( !$atts['subtitle_above'] && ! empty( $atts['subtitle'] ) )
						echo parent::getHeading('subtitle', $atts, 'slide-subtitle no-uppercase '. $atts['subtitle_class']);
					?>
