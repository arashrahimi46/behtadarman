	...
	function xstore_add_responsive_size_param( $settings, $value ) {
        ...
        $output = '<div class="xstore-rs-wrapper ' . esc_attr( $settings['param_name'] ) . '">'; 
            $output .= '<div class="xstore-rs-item desktop">';
                $output .= '<span class="xstore-rs-icon mtips mtips-top"><i class="dashicons dashicons-desktop"></i><span class="mt-mes">'.esc_attr('Global', 'xstore').'</span></span>';
            ...
            $output .= '<div class="xstore-rs-item tablet hidden">';
                $output .= '<span class="xstore-rs-icon mtips mtips-top"><i class="dashicons dashicons-tablet"></i><span class="mt-mes">'.esc_attr('Tablet', 'xstore').'</span></span>';
                $output .= '<input type="number" min="1" class="xstore-rs-input" data-id="tablet">';
            $output .= '</div>';

            $output .= '<div class="xstore-rs-item mobile hidden">';
                $output .= '<span class="xstore-rs-icon mtips mtips-top"><i class="dashicons dashicons-smartphone"></i><span class="mt-mes">'.esc_attr('Mobile', 'xstore').'</span></span>';
            ...
            $output .= '<input type="hidden" data-css_args="' . esc_attr( json_encode( $settings['css_args'] ) ) . '" name="' . esc_attr( $settings['param_name'] ) . '" class="wpb_vc_param_value xstore-rs-value" value="' . esc_attr( $value ) . '">';
        $output .= '</div>';
