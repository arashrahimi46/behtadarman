(function ($) {
    ...
    $(document).ajaxComplete(function (event, xhr, settings) {
        if ( xhr.status == 200 && xhr.responseText && settings.data !== undefined && 0 <= settings.data.indexOf( 'action=vc_edit_form' ) ) {
            if ( wp.media ) {
    ...
    $('#vc_ui-panel-edit-element').on('vcPanel.shown', function () {
        ...
        $('.vc-button-set-item').on('click', function () {
            ...
            if ( $button_set.hasClass('et-font_container') ) {
                $button_set.parent().find('.vc_font_container_form_field-' + $($button_set).data('type') + '-select').val(value).addClass('SADAS').trigger('change');
            }
        });
