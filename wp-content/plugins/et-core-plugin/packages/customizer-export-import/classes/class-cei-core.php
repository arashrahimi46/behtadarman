...
final class CEI_Core {
	...
	static public function controls_enqueue_scripts()
	{
		...
		wp_localize_script( 'cei-js', 'CEIl10n', array(
			'emptyImport'	=> __( 'Please choose a file to import.', 'xstore' )
		));
	...
	static private function _import( $wp_customize )
	{
		...
		if ( ! file_exists( $file['file'] ) ) {
			$cei_error = __( 'Error importing settings! Please try again.', 'xstore' );
			return;
		}
		...
		if ( 'array' != gettype( $data ) ) {
			$cei_error = __( 'Error importing settings! Please check that you uploaded a customizer export file.', 'xstore' );
			return;
		}
		...
		if ( $data['template'] != $template ) {
			$cei_error = __( 'Error importing settings! The settings you uploaded are not for the current theme.', 'xstore' );
			return;
		}
