...
$lightbox = etheme_get_option('product_gallery_lightbox_et-desktop');
$space = etheme_get_option('product_gallery_spacing_et-desktop');
$space = $gallery_type == 'full_width' ? 0 : $space;
...
if ( !$attachment_ids && !$has_video && apply_filters('etheme_single_product_variation_gallery', get_query_var('etheme_single_product_variation_gallery', false)) ) {
    $force_check = true;
...
$et_zoom = etheme_get_option( 'product_gallery_zoom_et-desktop' );
...
<?php if ( ! defined('DOING_AJAX') && ! isset($etheme_global['quick_view']) ): 
    ...
    if ( $lightbox ) $wrap_class .= ' with-pswp';
    if ( $vertical_slider && $gallery_slider && $attachment_ids ) $wrap_class .= ' swiper-vertical-images';
    if ( !$thumbs_slider ) $wrap_class .= ' product-thumbnails-as-grid';
    ?>
    <div class="swiper-entry swipers-couple-wrapper images images-wrapper woocommerce-product-gallery arrows-hovered <?php 
...
if ( $gallery_slider && ( count($attachment_ids) > 0 || $has_video || $force_check ) ){
    ...
    ?>
    <div class="<?php echo esc_attr($swiper_container); ?> <?php echo esc_attr( $class ); ?> swiper-control-top" data-effect="<?php echo apply_filters('single_product_main_gallery_effect', 'slide'); ?>" data-autoheight="true" data-space="<?php echo esc_attr($space); ?>" <?php if ( $gallery_type == 'full_width') echo ' data-loop="true" data-slides-per-view="1"'; ?> <?php if ( $force_check ) : ?> data-default-empty="true" <?php endif; ?>>
...
if($gallery_slider && $show_thumbs) {
    ...
    if ( $attachment_ids || $has_video || $force_check ) {
        $loop       = 0;
        $res_slides = !$vertical_slider && etheme_get_option('product_thumbnails_columns_et-desktop') ? etheme_get_option('product_thumbnails_columns_et-desktop') : 4;
        $columns    = apply_filters( 'woocommerce_product_thumbnails_columns', $res_slides );
        $ul_p_class = '';
        if (!$vertical_slider) {
            $ul_p_class .= 'swiper-container swiper-control-bottom';
            if ( !$thumbs_slider ) $ul_p_class .= ' swiper-container-grid';
        }
        else {
            $ul_p_class .= 'vertical-thumbnails-wrapper';
            if ( !$thumbs_slider ) $ul_class .= ' slick-vertical-slider-grid';
        }

        $ul_p_class .= ' columns-' . $res_slides;
        if (count($attachment_ids) + 1 <= $res_slides) $ul_p_class .= ' no-arrows';
        $ul_p_class .= (!$gallery_slider) ? ' noslider' : ' slider';

        $ul_p_class .= ( $force_check ) ? ' dt-hide mob-hide' : '';

        ?>
        <div class="<?php echo esc_attr($ul_p_class); ?>" <?php if (!$vertical_slider) : ?>data-breakpoints="1" data-xs-slides="3" data-sm-slides="<?php echo esc_attr( $res_slides ); ?>" data-md-slides="<?php echo esc_attr( $res_slides ); ?>" data-lt-slides="<?php echo esc_attr( $res_slides ); ?>" data-slides-per-view="<?php echo esc_attr( $res_slides ); ?>" data-clickedslide="1" data-space="<?php echo esc_attr($space); ?>" <?php endif;?>>
            <?php etheme_loader(); ?>
            <ul
                class="<?php echo esc_attr( $ul_class ); ?>"
                <?php if ($vertical_slider) echo 'data-slick={"slidesToShow":'.esc_attr( $res_slides ) . '}';?>>
                <?php
            ...
            <?php if ($vertical_slider && $thumbs_slider) { ?> <div class="swiper-vertical-navig"></div> <?php }
