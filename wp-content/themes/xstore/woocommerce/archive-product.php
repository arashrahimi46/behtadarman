...
<?php 	
	...
	if ( ($loop > 3 && ( $l['sidebar'] != 'without' || !$full_width ) ) || ( $view_mode == 'grid' && $loop > 5 ) ) {
		$class .= ' products-hover-only-icons';
	} 

?>

<?php if ( etheme_get_option( 'product_bage_banner_pos' ) == 4 ) { 
	etheme_category_header(); 
	do_action( 'woocommerce_archive_description' );
} ?>
<div class="<?php echo (!$full_width) ? 'container' : 'shop-full-width'; ?> sidebar-mobile-<?php etheme_option( 'sidebar_for_mobile' ); ?> content-page <?php echo esc_attr($class); ?>">
	<?php if ( etheme_get_option( 'product_bage_banner_pos' ) == 3 ) { 
		etheme_category_header(); 
		do_action( 'woocommerce_archive_description' );
	}  ?>
		...
		<div class="row">
			...
			<div class="content main-products-loop <?php echo esc_attr( $l['content-class'] ); ?>">
				...
				<?php if ( woocommerce_product_loop() ) : ?>
					...
					<?php if ( etheme_get_option( 'product_bage_banner_pos' ) == 1 ) { 
						etheme_category_header(); 
						do_action( 'woocommerce_archive_description' );
					} ?>

					<?php if ( etheme_get_option( 'top_toolbar' ) && woocommerce_products_will_display() ): ?>
					...
					<?php $search_content = etheme_get_option( 'search_results_content_et-desktop' ); ?>
			...
			<?php
				...
				if ( etheme_get_option( 'product_bage_banner_pos' ) == 2 ) { 
					etheme_category_header(); 
					do_action( 'woocommerce_archive_description' );
				} 
