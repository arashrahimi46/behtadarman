...
function et_custom_shortcodes_css($content) {
	...
	$et_wpbakery_css_module = etheme_get_option('et_wpbakery_css_module', false);
		...
		if ( isset( $shortcode['params'] ) && ! empty( $shortcode['params'] ) ) {
			foreach ( $shortcode['params'] as $param ) {
				if ( isset( $param['type'] ) && isset( $attr_array[ $param['param_name'] ] ) ) {
					...
					if ( isset($attr_array['et_uniqid']) ) {
						if ( $local_css != '' ) {
							$css['global'][] = '.' . $attr_array['et_uniqid'] . $prefix . '{ ' . $local_css . ' }';
						}
						if ( isset($attr_array['z_index'] ) ) {
							$css['global'][] = '.' . $attr_array['et_uniqid'] . ' { z-index: ' . $attr_array['z_index'] . '; }';
						}
					}
					...
					if ( 'css_editor' === $param['type'] && $et_wpbakery_css_module ) {
						...
						if ( isset($attr_array['et_uniqid']) ) {
							...
							if ( isset($attr_array['css_md'] ) ) {
								...
								$css['md'][] = str_replace($class, $attr_array['et_uniqid'] . $prefix, $attr_array['css_md']);
							}

							if ( isset($attr_array['css_sm'] ) ) {
								$class = vc_shortcode_custom_css_class( $attr_array['css_sm'] );
								$css['sm'][] = str_replace($class, $attr_array['et_uniqid'] . $prefix, $attr_array['css_sm']);
							}

							if ( isset($attr_array['css_xs'] ) ) {
								$class = vc_shortcode_custom_css_class( $attr_array['css_xs'] );
								$css['xs'][] = str_replace($class, $attr_array['et_uniqid'] . $prefix, $attr_array['css_xs']);
							}
...
if(!function_exists('etheme_columns_options')) {
	function etheme_columns_options() {
		...
		if ( etheme_get_option('et_wpbakery_css_module', false) ) { 
		...
		vc_add_params('vc_row_inner', array(
			array(
				...
				"group" => esc_html__('Design Options', 'xstore'),
				'value' => 'et_custom_uniqid_' . uniqid(),
			),
		) );
...
if( ! function_exists( 'et_rewrite_vc_google_font' ) ) {
	...
	function et_rewrite_vc_google_font( $fonts ){
        ...
        $et_fonts_list = '[' . $et_fonts_list . ']';

		$fonts = array_merge( $fonts, json_decode( $fonts_list ), json_decode($et_fonts_list) );
...
function etheme_product_name_render($atts) {
	...
	if ( ! class_exists('ETC\App\Controllers\Shortcodes') ) return;
...
function etheme_product_excerpt_render($atts) {
	...
	if ( strlen($options['short_descr']) > 0 && $atts['cutting'] != 'none') {
		...
		if ( strlen($options['excerpt']) < strlen($options['short_descr']) ) $options['excerpt'] .= $atts['symbols'];
	}
...
function etheme_product_buttons_render ($atts) {
	...
	ob_start(); ?>
	<footer class="footer-product2 <?php echo esc_attr( $footer_class ); ?>"><div class="footer-inner <?php echo esc_attr( $el_class ); ?>">
