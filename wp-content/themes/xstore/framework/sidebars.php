...
if(!function_exists('etheme_page_config')) {
	function etheme_page_config() {
		...
		if ( $page_id === $posts_page_id && etheme_get_option('only_blog_sidebar') ) {
			$layout['sidebar'] = etheme_get_option('blog_sidebar');
		}
		elseif ( !etheme_get_option('only_blog_sidebar') ) {
			$layout['sidebar'] = etheme_get_option('blog_sidebar');
		}
		else {
		...
		if ( is_singular('post') ) {
			$layout['sidebar'] = etheme_get_option('post_sidebar');
		}
		...
		if(class_exists('WooCommerce') && is_singular( "product" ) ) {
			...
			if ( get_query_var('etheme_single_product_builder', false) ) {
				$layout['sidebar'] = etheme_get_option('single_product_sidebar_et-desktop');
			}
		}

		$search_sidebar = etheme_get_option('search_page_sidebar_et-desktop');
		if ( is_search() ) {
			if ( get_query_var( 'etheme_header_builder', false ) ) {
				...
				if ( etheme_get_option('search_page_sidebar') ) $layout['sidebar-size'] = 0;
			}
		}
